...
| # | 工作 | 阻塞了什么 |
| --- | --- | --- |
| 1 | ~~`Grassland` 数据层 + 接 `Clock.day_ended`~~ **已完成** | 头牛挑草场、走失投放、产奶 |
| 2 | ~~牛接草场（消耗、效率、选点、挑草场）~~ **已完成** | 多日节奏 |
| 3 | ~~`SpeciesData` 抽出 `cow.gd` 的 @export~~ **已完成** | M3 全部调参 |
| 4 | ~~`cow.tscn` 预制体 + `CowData` + `HerdManager` 重写~~ **已完成** | 存档、走失、买牛 |
| 5 | `cow.gd` 拆分 | 群体力已拆出 `cow_forces.gd`；状态机与外观仍在 `cow.gd`（687 行） |
| 6 | ~~归栏判定收进 `Pen` + 60 s 宽限 + 出栏~~ **已完成** | 结算 |
| 7 | ~~`main/world/ui` 三场景拆分~~ **已完成**（重建入口已备好，尚未接到次日流程） | 次日重建 |
| 8 | ~~结算 UI + `tr()` + 藏文 TextServer 验证~~ **已完成** | M4 |
| 9 | ~~`SaveIO` + `GameState`~~ **已完成**（落盘往返、版本拒绝已验证） | M4 出口 |

1–2 是 GRASSLAND 的实现，3–5 是本文的重构，6–7 是 DAY_CYCLE 的实现，8–9 是 M4。

§6 的九项全部完成（2026-09-08）。M3 灰盒的结构性工作到此为止，剩下的是数值与手感。

**下一批**（按价值排序，都不阻塞别人）：

| 工作 | 说明 |
| --- | --- |
| 次日走 `reload_world()` | 重建入口已备好，`DaySettlement._on_slept()` 换过去即可。换之前先确认就地摆位够不够用 |
| `cow.gd` 继续拆 | 还有 687 行。状态机（约 200）与外观、施压响应可以再分。**不急**——拆的价值在调参时只碰一个文件，群体力已经拆出去了 |
| 走失的牛不入群 | 已按"不属于任何群"投放，但玩家把它推回群里的判定还没做（DAY_CYCLE §5.1 末段） |
| 死亡现场 | `GameState.death_marks` 已存，地图上还没有痕迹（DAY_CYCLE §5.2 第 4 条） |
| 一天的光色 | PRD §9 说这是最重要的美术投入，M3 唯一该做的"美术" |
